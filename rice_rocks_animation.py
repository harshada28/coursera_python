...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        global explosion_info, explosion_image, counter
        if self.animated:            
            t_center = list(explosion_info.get_center())
            image_size = explosion_info.get_size()           
            for counter in range(0, 25):
                t_center[0] += counter * image_size[0]
                
                counter = counter + 1
                canvas.draw_image(explosion_image, t_center, image_size, self.pos, image_size, 0)            
        else:            
            canvas.draw_image(self.image, self.image_center, self.image_size, self.pos, self.image_size, self.angle)
...
def group_group_collide(sprites1, sprites2):
    ...
    cnt = 0
    for sprite in grp1:
        if group_collide(sprites2, sprite):
            tmp.add(sprite)
            cnt += 1
...
def draw(canvas):
    ...
    global soundtrack, rock_group, missile_group, explosion_group
...
def rock_spawner():
    ...
    r2 = a_rock.get_radius()
    
    distance = dist(ship_pos, position)
    if distance >= r1 + r2 + 10 and len(rock_group) < 12:
